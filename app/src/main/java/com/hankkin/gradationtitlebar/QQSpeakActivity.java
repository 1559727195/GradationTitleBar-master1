...
public class QQSpeakActivity extends AppCompatActivity implements GradationScrollView.ScrollViewListener{
    ...
    private void initListeners() {
        ...
        vto.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            //当在一个视图树中全局布局发生改变或者视图树中的某个视图的可视状态发生改变时，addOnGlobalLayoutListener
            // 所要调用的回调函数的接口类
            @Override
            public void onGlobalLayout() {
                textView.getViewTreeObserver().removeGlobalOnLayoutListener(
                        this);
                height = ivBanner.getHeight();//600
                Log.e("robin debug","scrollview:change:height:" + height);

                scrollView.setScrollViewListener(QQSpeakActivity.this);
            }

//            现在需要使用getViewTreeObserver().addOnGlobalLayoutListener()来获得宽度或者高度
        });
    }
    ...
    public void onScrollChanged(GradationScrollView scrollView, int x, int y,
        ...
        if (y <= 0) {   //设置标题的背景颜色
            ...
            textView.setTextColor(Color.argb((int) alpha, 255,255,255));//   toolbar.setAlpha(alpha);
